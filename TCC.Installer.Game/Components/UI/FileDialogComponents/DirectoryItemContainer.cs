...
namespace TCC.Installer.Game.Components.UI.FileDialogComponents
{
    public class DirectoryItemContainer : KeyBindingActionContainer<FileDialogAction>
    {
        ...
        private readonly TCCScrollContainer scrollContainer;
        ...
        protected TCCButton ActionButton;
        ...
        public void NavigateToPreviousDirectory()
        {
            ...
            CurrentDirectory = PathExpansionPack.ConcatenateDirectoryPath(dirs.SkipLast(2).ToList());
        }
        ...
        public void HandleDirectoryChanged(ValueChangedEvent<string> value)
        {
            ...
            {
                UpdateItemList();
                SelectedItem = PathExpansionPack.GetPreviousPathDirectoryInNewPath(value.OldValue, value.NewValue);
            }
            catch
            {
        ...
        private void UpdateItemList(bool forceUpdate = false)
        {
            ...
            if (CurrentDirectory.Length > 0)
            {
                ...
                foreach (var d in directories)
                    fileFillFlowContainer.Add(GetNewDrawableItem(PathExpansionPack.GetIndividualItemName(d.Name),
                        PathItemType.Directory));
                foreach (var f in files)
                    fileFillFlowContainer.Add(GetNewDrawableItem(PathExpansionPack.GetIndividualItemName(f.Name), PathItemType.File));
            }
            else
            {
                ...
                foreach (var d in drives)
                    if (d.IsReady)
                        fileFillFlowContainer.Add(GetNewDrawableItem(
                            PathExpansionPack.GetIndividualItemName(d.Name.Remove(d.Name.Length - 1)), PathItemType.Volume));
            }
        ...
        {
            return (int)(DrawHeight
                         / (DrawableItem.DefaultHeight + ItemSpacing)
                         + 0.5);
        }
